function [output] = model_SWE_NG(input)
% Estimates the non-Gaussian SVAR small working example model as described in the Appendix of the paper 
...
%     - input.Ainit: Initial AR parameter matrix A. 
...
for irep = 1:ndraws*nthin+nburn
   waitbar(irep/(ndraws*nthin+nburn),hwaitbar);
    ...
    for i = 1:n % Sample row by row via Gibbs Sampler:
