function [ check, logprior ] = prior_A_BH19( Ai, i )
...
elseif i == 3
    ...
    b_qp = -Ai(3); % beta(qp): short-run price elasticity of oil demand (sign: negative)
...
elseif i== 5
    ...
    if  rhostar>0  && chi>0 &&  rhostar<1 && chi<1
        ...
        % Add Jacobian of transformation (numerically calculated)
        % x = [chi,rhostar]
        feval=@(x)[ 1./sqrt( x(2)/(1-x(2))*x(1)^(-2)*1.3 ) ; -x(1)./sqrt( x(2)/(1-x(2))*x(1)^(-2)*1.3 )];
        J = Gradp(feval,[chi;rhostar]);
        ldetJ = LogAbsDet(J);
        logprior = log(p1)+log(p2) - ldetJ;
